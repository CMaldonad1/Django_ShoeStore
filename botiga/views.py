...
def cataleg(request):
    ...
    categories=Categoria.objects.raw("SELECT * FROM `botiga_categoria` bc LEFT JOIN (SELECT jerarquia_id, count(jerarquia_id) as \"count\" FROM `botiga_categoria` where jerarquia_id is not null GROUP BY jerarquia_id) cnt on bc.id=cnt.jerarquia_id; ")
